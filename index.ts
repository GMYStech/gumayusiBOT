...
import { showComponent, replayChampionInfoURL } from './lib/utils.js'
import {
...
const createChannels: { vc: VoiceChannel; tc: TextChannel }[] = []
...
client.on('ready', async (client) => {
  ...
  textChannels.forEach((tc) => {
    if (tc.type !== ChannelType.GuildText) {
      return
    }
    const vcId = tc.topic
    ...
    if (vc.type === ChannelType.GuildVoice) createChannels.push({ vc, tc })
  })
})
//メンションに「Hi!」と返信
